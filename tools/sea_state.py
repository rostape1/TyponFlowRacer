...
A rival from static/races/ (AIS, scored the same way; --control, default the race's first AIS
rival, Wowla in 2026) is the control: same water, same minutes. If chop costs us and not them, it is us, not the sea.
"""
...
def parse_pitch():
    """10 Hz pitch for every race in tools/regattas.json (epoch-s index; race = e.g. BBS2026-R1)."""
    out = []
    for R in rt.race_list():
        rid, day = R['id'], R['day']
        t0, t1 = rt._utc_s(day, R['start']), rt._utc_s(day, R['finish'])
        rows = []
        for f in sorted(glob.glob(os.path.join(LOGS, f'nmea_{day}_*.txt'))):
            for line in open(f, errors='replace'):
                ...
                try:
                    ...
                    pass
        if not rows:
            print(f'{rid}: no pitch ($YXXDR) in the logs', file=sys.stderr)
            continue
...
def windows(P, races, control=None):
    """Upwind 5-min windows: sea state, TWS, Typon's and the control rival's median upwind % of ORC.
    control = rival name, or None for the race's first AIS rival."""
    rows = []
    for race in races:
        rid = race['id']
        boats = {b['name']: pd.DataFrame(b['pts'], columns=rt.FIELDS) for b in race['boats']}
        ctl = control or next((b['name'] for b in race['boats'] if b['source'] == 'ais'), None)
        x = P[P.race == rid]
        for w0 in np.arange(race['start'] / 1000, race['finish'] / 1000 - WINDOW_S, WINDOW_S):
            ...
            rows.append(dict(race=rid, t=w0, tws=up.tws.median(), pitch=spread, period=period,
                             typon=up.pct.median(), control=wu))
...
def main():
    ...
    ap.add_argument('--cache', action='store_true', help='reuse tools/polar_out/pitch.pkl (rebuild after adding races)')
    ap.add_argument('--control', help='rival to compare against (default: each race\'s first AIS rival)')
    args = ap.parse_args()
    if args.cache and os.path.exists(CACHE):
        ...
        P = parse_pitch()
        P.to_pickle(CACHE)
    W = windows(P, rt.load_tracks(), args.control)
    ...
    print(W.groupby('race').agg(windows=('t', 'size'), tws=('tws', 'median'), pitch=('pitch', 'median'),
                                period=('period', 'median'), typon=('typon', 'median'), control=('control', 'median'))
          .round(1).to_string())
    ...
    for b, d in W.groupby('band', observed=True):
        ...
        for half, h in (('calmer', d[d.pitch <= m]), ('rougher', d[d.pitch > m])):
            rows.append(dict(band=b, half=half, windows=len(h), pitch=h.pitch.median(), period=h.period.median(),
                             typon=h.typon.median(), control=h.control.median(), control_n=h.control.notna().sum()))
    print(pd.DataFrame(rows).round(1).to_string(index=False))

    for col in ('typon', 'control'):
        b, c, n = fit(W, col)
