...
- **Route optimizer** — isochrone-based optimal routing through time-varying currents and wind. Swan 47 polars with adjustable performance factor. Click start/end on map, shows fastest path colored by current benefit (green=favorable, red=adverse) with time labels. Correctly uses forecast-time environmental data when routing in forecast mode. Land detection via NOAA ENC S-57 polygons
