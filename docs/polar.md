...
| Paddlewheel by tack | **Heeled to port (starboard tack) it exaggerates speed changes: response ×0.72, reads right near 5.4 kn, ~+0.4 kn (6%) high at 6.5 kn.** Measured without the current: over a minute on a steady course current is constant, so ΔGPS speed / Δpaddlewheel is the wheel's response. Heeled to port 0.66–0.77 (after tacks, n = 16; steady 90–150 s boards, n = 40+; light air and breeze; unchanged with 9–15 s smoothing, so not wave jitter), heeled to starboard and downwind ~1.0; its minute-to-minute wobble is 2.2× port's against 1.3× for GPS. The level, with the slope held, from the multi-heading current fit: pivot 5.1–5.7 kn over 7 window choices (+0.33..+0.54 kn at 6.5), and it matches GPS − paddlewheel reading −0.40..−0.49 kn on starboard beats in all six races. `paddle_starboard()` applies it to the raw reading first (ramped in over 5–10° of heel). A per-tack *scale* fitted against GPS was tried first and dropped: tacks are sailed in different water, and it gave 0–12% by window | corrected (2026-09-26) |
...
Its **Methodology** button shows `static/methodology.md`: this document's method in crew language
(calibration, leeway, current, scoring, rivals, review metrics, trust). Keep it in step when a
calibration number or a scoring rule here changes.
...
| `pointing` | Bow against bow and track against track with each AIS rival that transmits its heading (Wowla, Frequent Flyer), same moment, same tack, within 1 nm, the two tacks averaged so a wind-direction difference or a compass offset cancels. Series: Wowla's track ~2° higher than ours (5° in 10–13 kn, 1° in 13–16, level in 16+); per beat 1–5° (R5 closest), noisy where one tack has few reports. Rival leeway from its heading is unreliable (their sensor). |
