...
if (_routeDetailsBtn) {
    _routeDetailsBtn.addEventListener('click', () => {
        ...
        for (const pt of path) {
            const d = new Date(pt.timeMs);
            const timeStr = d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            const awa = Math.round(Math.abs(pt.awa));
            html += `<tr><td>${timeStr}</td><td>${pt.bsp.toFixed(1)}</td><td style="color:${twsColor(pt.tws)}">${pt.tws.toFixed(1)}</td><td>${Math.round(pt.twa)}\u00b0</td><td style="color:${twsColor(pt.aws)}">${pt.aws.toFixed(1)}</td><td style="color:${awaColor(pt.awa)}">${awa}\u00b0</td></tr>`;
        }
        html += '</tbody></table>';
