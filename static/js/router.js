...
// --- Water detection from US Census TIGER/Line land polygons ---
// Pre-computed binary grid for O(1) lookups, with polygon fallback.
// Grid generated by generate_land_mask.py at ~550m resolution.
...
function _isLand(lat, lon) {
    // Fast path: pre-computed grid lookup (O(1))
    if (_landGrid) {
        const g = _landGrid;
    ...
    // Fallback: polygon point-in-polygon test
...
// When using pre-computed grid, buffer is already baked in — single lookup suffices.
...
const TIME_STEP_S = 120;        // 2-min steps near land
const TIME_STEP_OPEN_S = 300;   // 5-min steps on open water
const MAX_TIME_S = 86400;       // 24 hours
...
function computeRoute(startLat, startLon, endLat, endLon, startTimeMs, perfFactor, onProgress) {
    ...
    return store.preload(startTimeMs, hoursNeeded, startLat, startLon, endLat, endLon).then(async info => {
        ...
        while (elapsedS < MAX_TIME_S && step < maxSteps) {
            ...
            // Adaptive time step: 2-min near land, 5-min on open water
            ...
            if (step % 5 === 0) {
                ...
                if (onProgress) onProgress(elapsedS, MAX_TIME_S);
            }
